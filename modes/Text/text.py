...
text = Blueprint("text", __name__, static_folder="static",
                 template_folder="templates") #It is to organize the group of codes.
...
def text_encode():
    if os.path.exists(current_app.config['TEXT_CACHE_FOLDER']):
        shutil.rmtree(
            current_app.config['TEXT_CACHE_FOLDER'], ignore_errors=False) #here it deletes the folder and the previous file.
    else:
        print("Not Found")
